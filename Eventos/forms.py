...
class EventoForm(forms.ModelForm):
    ...
    texto_boton = forms.CharField(max_length=100, widget=forms.URLInput(attrs={'class': 'form-control'}), required=False, label='Texto del botón')
    link = forms.CharField(max_length=300, widget=forms.TextInput(attrs={'class': 'form-control'}), required=False, label='Link de redirección')
    activo = forms.BooleanField(widget=forms.CheckboxInput(attrs={'class': 'form-control'}), label='¿Activo?')

    class Meta:
        model = Evento
        fields = ['nombre', 'descripcion', 'imagen', 'texto_boton', 'link', 'activo']
